package com.base.export.implement;
...
import com.base.export.entity.DataBlockInfo;
 ...
 */
@Repository
public class ClsExport implements com.base.export.interfaces.IExport {
	...
	public int Export() {	
		...
		if(iRet <0 ){invalid.Invalid(iRet);return iRet;}
		iRet = this.drawTitle();
		if(iRet <0 ){invalid.Invalid(iRet);return iRet;}
		iRet = this.drawHead();
		if(iRet <0 ){invalid.Invalid(iRet);return iRet;}
	...
	private int writeToFile(){
		try {
			...
			this.setFILE_PATH("D:"+File.separator+"zhao");
